...
function subst_env_vars {
    ...
    declare -a subst=(
      ...
      "K8S_CP_LABEL:=__.Values.k8sCpLabel__"
      "CAPA_DIAGNOSTICS_ADDRESS=:__.Values.manager.diagnosticsAddress__"
      "CAPA_INSECURE_DIAGNOSTICS=__.Values.manager.insecureDiagnostics__"
      "CAPA_LOGLEVEL=__.Values.manager.logLevel__"
      "EXP_MACHINE_POOL=__.Values.manager.featureGates.machinePool__"
      "CAPA_EKS=__.Values.manager.featureGates.eks__"
      "CAPA_EKS_IAM=__.Values.manager.featureGates.eksEnableIAM__"
      "CAPA_EKS_ADD_ROLES=__.Values.manager.featureGates.eksAllowAddRoles__"
      "EXP_EKS_FARGATE=__.Values.manager.featureGates.eksFargate__"
      "EVENT_BRIDGE_INSTANCE_STATE=__.Values.manager.featureGates.eventBridgeInstanceState__"
      "AUTO_CONTROLLER_IDENTITY_CREATOR=__.Values.manager.featureGates.autoControllerIdentityCreator__"
      "EXP_BOOTSTRAP_FORMAT_IGNITION=__.Values.manager.featureGates.bootstrapFormatIgnition__"
      "EXP_EXTERNAL_RESOURCE_GC=__.Values.manager.featureGates.externalResourceGC__"
      "EXP_ALTERNATIVE_GC_STRATEGY=__.Values.manager.featureGates.alternativeGCStrategy__"
      "TAG_UNMANAGED_NETWORK_RESOURCES=__.Values.manager.featureGates.tagUnmanagedNetworkResources__"
      "EXP_ROSA=__.Values.manager.featureGates.rosa__"
    )
    for SUB in "${subst[@]}" ; do
       local WHAT="${SUB%=*}"
       local WITH="${SUB##*=}"
...
SRC_CC="$SRC/openshift/infrastructure-components-openshift.yaml"
[ -f "$SRC_CC" ] || SRC_CC="$SRC/openshift/infrastructure-components.yaml"
