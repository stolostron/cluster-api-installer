...
func ValidatingWebhook(namespace string) *admissionregistration.MutatingWebhookConfiguration {
	fail := admissionregistration.Fail
	none := admissionregistration.SideEffectClassNone
	path := "/mutate"
	namespacedScope := admissionregistration.NamespacedScope
				...
				Rules: []admissionregistration.RuleWithOperations{
					{
						Rule: admissionregistration.Rule{
							...
							APIVersions: []string{clusterv1.GroupVersion.Version},
							Resources:   []string{"*"},
...
var _ = BeforeSuite(func() {
	...
	testEnv = &envtest.Environment{
		CRDDirectoryPaths:     []string{filepath.Join("..", "..", "charts", "cluster-api", "crds")},
		ErrorIfCRDPathMissing: true,
		WebhookInstallOptions: envtest.WebhookInstallOptions{
			MutatingWebhooks: []*admissionregistration.MutatingWebhookConfiguration{
	...
	err = SetupWebhookWithManager(cfg, mgr)
	Expect(err).NotTo(HaveOccurred())
